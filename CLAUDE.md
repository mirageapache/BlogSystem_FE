...
Forms use **react-hook-form** v7 with **zod** schemas via `@hookform/resolvers/zod` (Phase 2.1). Validation schemas live in `src/schemas/` (e.g. `src/schemas/auth.ts` for sign-in/up, find/reset password) — error message strings are kept in Traditional Chinese and matched to the existing test assertions. The shared `components/form/FormInput` is RHF-compatible: pass `registration={register('field')}` and `errorMsg={errors.field?.message}` (it no longer takes `value`/`setValue`/`name`/`handleEnter`). Submit via `<form onSubmit={handleSubmit(onSubmit)}>` with a `type="submit"` button; native Enter submits, so the old `handleEnter` prop is gone.

zod is pinned to **v3** (3.23.x), not v4: the repo is still on **TypeScript 4.9.5**, and zod v4's `.d.cts` type defs require TS ≥5.5 (they fail to parse on 4.9). For the same reason `@hookform/resolvers` is pinned to **v3** (v5 pulls zod-v4 standard-schema types). Upgrading TypeScript to 5.x would unblock zod v4 but is a separate, cross-cutting change.

`EditProfilePage` is **not yet migrated** to RHF (Phase 2.1 batch 2 — it has avatar/FormData complexity); its name/account/email fields are temporary inline controlled `<input>`s. `src/utils/formValidates.ts` (the old manual-validation helpers) was deleted; the `validator` package is now unused and is a Phase 2.3 removal target.
