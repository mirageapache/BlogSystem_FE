...
describe('測試PostList元件', () => {
  ...
  test('測試貼文清單顯示', () => {
    render(<PostListDynamic postListData={mockPostData} isLoading={false} atBottom={false} />);
    // 檢查渲染的 PostItem 元素數量是否與 mockPostData 長度一致
    expect(screen.getAllByText('Post Item').length).toBe(mockPostData.length);
  ...
  test('測試沒有更多貼文顯示', () => {
    render(<PostListDynamic postListData={mockPostData} isLoading={false} atBottom />);
